...
def _parse_label_(label_line):
	xmin = math.floor(float(label_line.split(' ')[0]))
	ymin = math.floor(float(label_line.split(' ')[1]))
	xlen = math.floor(float(label_line.split(' ')[2]))
	ylen = math.floor(float(label_line.split(' ')[3]))
	class_id = int(float(label_line.split(' ')[4]))
	face_id = int(float(label_line.split(' ')[5]))
	
	xmax = xmin + xlen
	ymax = ymin + ylen	
	return xmin, ymin, xmax, ymax, class_id, face_id

class multi_source_data(imdb):
    def __init__(self, image_set, data_path):
        imdb.__init__(self, 'multi_source_' + image_set)
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'album')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_fashion_annotation(index)
    ...
    def _load_fashion_annotation(self, index):
	"""
	Load the annotation info from text file 
	"""
	filename = os.path.join(self._data_path, 'Labels',  index + '.txt')
	labels = open(filename, 'r').readlines()
	img_fn = os.path.join(self._data_path, 'Images',  index + '.jpg')
    ...
    def _get_voc_results_file_template(self):
        ...
        path = os.path.join(
            ...
            'results',
            filename)
	if not os.path.exists(os.path.join(self._data_path, 'results')):
		os.makedirs(os.path.join(self._data_path, 'results'))
        return path
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} VOC results file'.format(cls_ind)
